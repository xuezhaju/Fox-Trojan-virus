        ...
        try:
            ...
            if save_path:
                ...
                if os.path.isdir(save_path) or save_path.endswith('\\'):
                    # 新格式: screenshot[ip][name][time].jpg
                    ip_part = f"[{self.host}]" if hasattr(self, 'host') else ""
                    save_path = os.path.join(
                        save_path,
                        f"screenshot{ip_part}_{time.strftime('%Y%m%d_%H%M%S')}.jpg"
                    )
                # 确保目录存在
                ...
                # 新格式: screenshot[ip][name][time].jpg
                ip_part = f"[{self.host}]" if hasattr(self, 'host') else ""
                save_path = f"screenshot{ip_part}_{time.strftime('%Y%m%d_%H%M%S')}.jpg"
